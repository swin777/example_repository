...
const is3D = document.getElementById("switch") as HTMLInputElement; //3D 건물 표시 설정하는 스위치
...
function getExtrussionLayers(map: ktGms.Map) {
    return map.getStyle().layers.filter((layer:any) => {if(layer.type === "fill-extrusion" || layer.metadata === "group:area_3d" ) return layer;})
}

//opacity 값에 따라 건물을 3d로 보여줄지, 2d로 보여줄지 설정합니다
function switchMap(opacity: number, layers: any) {
    switchExtrusionVisibility(map, layers, opacity);
}

//opacity값으로 3D 건물 레이어의 가시성을 설정합니다
function switchExtrusionVisibility(map: ktGms.Map, layers: any, opacity: number) {
...
function changeBuildingColor(map:ktGms.Map, layers: any, color:string) {
