...
// kt연구개발센터 사옥
const origin = [127.029414, 37.471401];

// kt광화문 사옥
const destination = [126.978916, 37.572020];

let coordinates = []; // GeoJSON 객체에 들어가는 좌표 데이터
let animation; // 애니메이션 정지 / 시작을 위한 변수
let startTime = 0; // 진행 정도 계산을 위한 변수
let progress = 0; // 진행 정도 계산을 위한 변수
let resetTime = false; // 애니메이션 재시작 시 진행 정도 재계산을 위한 변수
const end = 8000; // 갱신 속도 계산을 위한 변수(숫자가 작을수록 갱신속도 빠름)
const pauseButton = document.getElementById("pause"); // button DOM 접근

// lineString Geometry 스타일 생성
let style = new ktGms.style.LineStyle( 
    {
...
map.on("load", () => {
    ...
    let layer = new ktGms.layer.LineLayer("line-animation", style, source);
    layer.addTo(map);
    ...
    function animateLine() {
        ...
        if (progress > end) { // 도착 시 초기화
            ...
            coordinates = [];
            // 좌표를 비우고 지도 업데이트
            map.getSource("line").setData(new ktGms.geometry.LineString(coordinates,{}));
