...
//클러스터 커스텀 하기 위한 마커 타입을 정의합니다
type markerObjectType = {[key:number]:ktGms.overlay.Marker}
...
map.on("load", () => {
    ...
    //커스텀 클러스터 (마커)
    const markers:markerObjectType = {}; //총 마커
    let markersOnScreen:markerObjectType = {}; //화면에 보이는 마커

    //지도 움직일 때마다 마커를 업데이트 해줄 함수
    function updateMarkers() {
        const newMarkers:markerObjectType = {};
        const features = map.querySourceFeatures("custom-cluster-source");

        for (let i = 0; i < features.length; i++) {
            const property:propertyType = features[i].properties;
            const coords = features[i].geometry.coordinates;
            if (!property.cluster) continue;
    ...
    map.on("data", (e:ktGms.event.MapMouseEvent) => {
