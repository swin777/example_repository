...
let clickedStateId = null; //선택된 구역의 id

map.on("load", () => {
	// 서울시 행정동 구역의 geoJson 데이터로 소스를 생성합니다.
	const source = new ktGms.source.GeoJSONSource("states", {
        data: "https://map.gis.kt.com/mapsdk/data/seoul.geojson",
        generateId: true,
    });
    ...
    map.onLayer(ktGms.event.MapMouseEvent.eventName.click, "state-fills", (e) => {
        // 마우스 아래의 Feature에 대한 "feature-state"입니다.
        if (e.features.length > 0) {
            // 행정동 정보가 포함된 infoWindow를 표시합니다.
            new ktGms.overlay.InfoWindow().setLngLat(e.lngLat).setHTML(e.features[0].properties.SIG_KOR_NM).addTo(map);
